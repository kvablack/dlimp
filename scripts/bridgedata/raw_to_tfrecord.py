...
def process_images(path):  # processes images at a trajectory level
    ...
    for missing in CAMERA_VIEWS - set(d.keys()):
        d[missing] = [""] * len(image_paths[0])  # empty string is a placeholder for missing images
...
def create_tfrecord(paths, output_path, tqdm_func, global_tqdm):
    ...
    for path in paths:
        try:
            ...
            assert len(out["actions"]) == len(out["obs"]["state"]) == len(out["obs"]["images0"])

            example = tf.train.Example(
                features=tf.train.Features(
                    feature={k: tensor_feature(v) for k, v in dl.transforms.flatten_dict(out).items()}
                )
            )
...
def main(_):
    ...
    with Pool(FLAGS.num_workers) as p:
        train_paths, val_paths = zip(*p.map(partial(get_traj_paths, train_proportion=FLAGS.train_proportion), paths))
